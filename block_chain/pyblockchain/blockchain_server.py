...
def consensus():
    ...
    return jsonify({'repleced': replaced}), 200

@app.route('/amount', methods=['GET'])
def get_total_amount():
    block_chain = get_blockchain()
    blockchain_address = request.args['blockchain_address']
    logger.info({'action': 'get_total_amount', 'target': blockchain_address, 'amount': block_chain.calculate_total_amount(blockchain_address)})
