...
class BlockChain(object):
    ...
    def create_transaction(self, sender_blockchain_address, recipient_blockchain_address, value,
        ...
        is_transacted = self.add_transaction(sender_blockchain_address, recipient_blockchain_address, value,
                        sender_public_key, signature)
    ...
    def valid_chain(self, chain):
        ...
        while current_index < len(chain):
            ...
            if block['previous_hash'] != self.hash(pre_block):
                logger.info({'action': 'valid_chain', 'status': 'invalied previou hash'})
                return False
            
            if not self.valid_proof(block['transaction'],
                                block['previous_hash'],
                                block['nonce'],MINING_DIFFCULTY):
                logger.info({'action': 'valid_chain', 'status': 'invalied nonce calc'})
                return False
        ...
        for node in self.neighbours:
            ...
            if response.status_code == 200:
                ...
                if chain_length > max_length and self.valid_chain(chain):
                    ...
                    logger.info({'action': 'resolve_conflicts', 'status': 'not find max length'})
            else:
                logger.info({'action': 'resolve_conflicts', 'status': 'request failed'})
