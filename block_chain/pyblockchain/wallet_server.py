...
def create_transaction():
    ...
    if not all(k in request_json for k in required):
        return jsonify({'message': 'missed values'})
...
def calculate_amount():
    ...
    if not all(k in request.args for k in required):
        logger.warning({'action': 'calculate_amount', 'message': 'parameter missed'})
    ...
    if response.status_code == 200:
        ...
        logger.info({'action': 'calculate_amount', 'amount': total})
        return jsonify({'message': 'success', 'amount': total}), 200
    logger.warning({'action': 'calculate_amount', 'backend_error_code': response.status_code})
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--port', default=8080, type=int, help='port to listning on')
